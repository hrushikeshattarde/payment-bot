...
- **[docs/LOCAL_RUN.md](docs/LOCAL_RUN.md) — start here to run it for real, locally:
...
payment-bot-demo    # run both skills end-to-end on a sample email, fully mocked
```

To run against the real inbox and real Transport Pro — drafting only, never sending — see
...
* **Sending email.** Deliberate: the bot creates Gmail drafts and a human presses Send.
  `GmailApiClient.send_reply` raises.
* A real Slack client and the Approve/Edit/Reject callback — those belong with the AWS
  deployment (needs a public endpoint); see [docs/AWS_DEPLOYMENT.md](docs/AWS_DEPLOYMENT.md).
  Locally, escalations go to the console report and the JSON log.
