...
def test_rate_verification_passes_the_gate_on_live_api_shapes() -> None:
    ...
    # File history is fetched with the internal record id from payment_information.
    assert any("recordId=1302556" in u for u in transport.data_urls())


@pytest.mark.integration
def test_unknown_load_is_blocked_by_the_gate_and_never_sends() -> None:
    """A load the API does not have must fail closed, not be answered with a guess.

    Every Transport Pro read returns the ``{"ok": false, "error": …}`` envelope, so nothing
    reaches the grounding ledger. The scripted model ignores those failures and submits its
    draft anyway — exactly the hallucination case the gate exists for — and the gate blocks
    it on both authorization and grounding rather than emailing the carrier.
    """
    ...
    assert result.gate_result is not None and not result.gate_result.allowed
    failed = {c.name for c in result.gate_result.checks if not c.passed}
    assert failed == {"authorization", "grounding"}
    # Not one figure in the draft traced to a tool result.
    grounding = next(c for c in result.gate_result.checks if c.name == "grounding")
    assert "150" in grounding.detail and "4500" in grounding.detail and "4650" in grounding.detail
