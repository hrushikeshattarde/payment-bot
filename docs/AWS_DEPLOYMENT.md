...
| **Transport Pro API** (7-digit loads) | Load summary (§4.3.0 payload), dispatch history, settlement entries, file history, NOA/factoring, carrier search | Provider-specific (API key or OAuth2) — **open dependency, PRD §9** | Read-only | Logical endpoints your `TransportProClient` needs: load-by-id, dispatch-history, settlement-entries, file-history, noa/factoring, search-by-carrier | API key / OAuth creds in SSM/Secrets Manager |
...
> Transport Pro and QuickBooks specifics (base URL, exact auth, field names) are the
> **Phase-0 open dependencies** in PRD §9 — confirm them before implementing the real
> adapters. The QBO (6-digit) path and carrier-name lookup are not yet built in code.

### 5.3 Config → secret mapping

| Setting (`PAYBOT_*` / secret) | Source in AWS |
|---|---|
| `PAYBOT_MODEL_FAST`, `PAYBOT_MODEL_DRAFT`, `PAYBOT_AWS_REGION`, `PAYBOT_ROLLOUT_PHASE`, thresholds, channels | Lambda env vars or SSM plaintext params |
| Slack bot token, Slack signing secret | SSM `SecureString` / Secrets Manager |
| Gmail service-account JSON | Secrets Manager |
| Transport Pro credentials | SSM `SecureString` / Secrets Manager |
...
- [ ] Transport Pro API base URL, auth, and endpoints confirmed → implement `TransportProClient`.
