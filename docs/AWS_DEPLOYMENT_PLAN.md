...
| `PAYBOT_ALLOW_FACTORING`, `PAYBOT_SENSITIVE_BANK_REPLIES`, `PAYBOT_SENSITIVE_NOA_REPLIES`, `PAYBOT_NOA_ATTACHMENT_REPLIES`, `PAYBOT_FACTORING_PRENOA_REPLIES` | SSM `/paybot/policy/*` | **Policy switches — changing them should be deliberate and audited**, hence Parameter Store with change history, not plain env. The last two were absent from the template until 2026-08-15, so a deployed stack silently ran them OFF while `.env` had them on: every NOA-attached rate verification escalated, and roster-verified factors on loads with no factor of record were denied outright. Both default `true` in the template now — a policy switch that only exists locally is not a policy |
